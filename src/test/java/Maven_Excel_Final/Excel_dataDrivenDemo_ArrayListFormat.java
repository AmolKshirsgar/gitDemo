...
//WAP accessing excel data with array concept and use this method in other class as per requirements
...
public class Excel_dataDrivenDemo_ArrayListFormat 
{
	...
	public ArrayList<String> getData(String testCaseName) throws IOException
	{
		...
		for(int i=0;i<sheets;i++)
		{
				...
				
			{
				...
				{
					Row cr=rows.next();
					if(cr.getCell(column).getStringCellValue().equalsIgnoreCase(testCaseName))
					{
						Iterator<Cell> cvalue=cr.iterator();
						while(cvalue.hasNext())
						{
							//	a.add(cvalue.next().getStringCellValue()); for string excel data only
							
							
							//beow code will have changes if, excel dat is having numeric values along string
							Cell c=cvalue.next();
